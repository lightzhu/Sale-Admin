...
const Repertory = ({ title, list, loading, loadMore, setImageList }) => {
  ...
  const content = (
    <div className='tools'>
      <Button size='small' block onClick={editPic}>
        管理图片
      </Button>
      <Button size='small' block>
        删除商品
      </Button>
      <Button size='small' block type='danger'>
        停售商品
      </Button>
    </div>
  )
  return (
    <div className={styles.listBox}>
      <div className={styles.head}>
        <h5>{title}</h5>
        <Link to='/commodity/commodity_add'>
          <Button type='primary'>上架商品</Button>
        </Link>
      </div>
      <List
          ...
          <List.Item
            ...
            <Skeleton avatar title={false} loading={item.loading} active>
              <List.Item.Meta
                ...
                title={
                  <a href='https://i.ebayimg.com/thumbs/images/g/yc4AAOSwQN5ag2m8/s-l225.webp'>
                    {item.product}
                  </a>
                }
              />
